...
from Helper.func_helper import db_connection, redis_connection
...
async def check(conn, cursor, data):
    if data.get("token"):
        if data["token"] is not None:
            query = "SELECT user_id, phone FROM ERS.dbo.tokens WHERE token = ?"
            ...
            if res is None:
                ...
                return True, "", {"user_id": res[0], "phone": res[1], }
        else:
            cursor.close()
            conn.close()
...
@app.post("/ers_api/update_user_info_file")
async def update_user_info_file(
    ...
    file_path = f"D:/WebSites/ERS/Media/UserPic/{pic.filename}"
    last_path = f"D:/WebSites/ERS/Media/UserPic/{last_pic}"
...
@app.get("/ers_api/get_user_pic/{filename}")
async def get_user_pic(filename: str):
    file_path = os.path.join('D:/WebSites/ERS/Media/UserPic/' + filename)
    return FileResponse(file_path, filename=filename)


@app.get("/ers_api/pdf_nootbook/{filename}")
async def get_pdf_nootbook(filename: str):
    file_path = os.path.join('D:/WebSites/ERS/Docs/' + filename)
...
@app.get("/ers_api/nootbook_image/{filename}")
async def get_notebook_pic(filename: str):
    file_path = os.path.join('D:/WebSites/ERS/Pics/NoteBook/' + filename)
    return FileResponse(file_path, filename=filename)


@app.get("/ers_api/get_pic_info/field/{filename}")
async def get_pic_info_field(filename: str):
    file_path = os.path.join('D:/WebSites/ERS/Pics/Field/' + filename)
    return FileResponse(file_path, filename=filename)
...
async def get_report(phone: str):
    conn, cursor = await db_connection()
    query = 'SELECT user_id FROM ERS.dbo.stu WHERE phone = ?'
    ...
    if res is None:
        ...
        query = 'SELECT quiz_id, state FROM ERS.dbo.quiz_answer WHERE user_id = ? order by edited_time desc'
        ...
        if len(res_score) < 7:
            ...
            if res_score[-1][1] != 2:
                raise HTTPException(status_code=321, detail="در حال حاضر آزمون‌های شما به پایان نرسیده است.")
    file_path = os.path.join('D:/WebSites/ERS/Reports/', phone, 'Report.pdf')
    folder_check = os.path.join('D:/WebSites/ERS/Reports/', phone)
    if os.path.exists(file_path):
...
async def get_pic_info_field(filename: str):
    file_path = os.path.join('D:/WebSites/ERS/Pics/Quiz/' + filename)
    if os.path.exists(file_path):
        return FileResponse(file_path, filename=filename)
    else:
        raise HTTPException(status_code=404, detail="File not found")
...
async def get_quiz_pic(filename: str):
    file_path = os.path.join('D:/WebSites/ERS/Quiz/' + filename)
    if os.path.exists(file_path):
        return FileResponse(file_path, filename=filename)
    else:
        raise HTTPException(status_code=404, detail="File not found")

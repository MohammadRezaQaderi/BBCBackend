...
def select_con_info(conn, cursor, user_id):
    query = 'SELECT con_id, phone, first_name, last_name, sex, ins_id, hCon_id FROM ERNew.dbo.con WHERE user_id = ?'
    res_con = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=user_id)
    query = 'SELECT name, logo FROM ERNew.dbo.ins WHERE user_id = ?'
    res_ins = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=res_con[5])
    query = 'SELECT first_name, last_name FROM ERNew.dbo.hCon WHERE user_id = ?'
    res_hCon = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=res_con[6])
    token = str(uuid.uuid4())
    info = {"phone": res_con[1], "user_id": user_id, "id": res_con[0], "first_name": res_con[2],
            "last_name": res_con[3], "role": "con", "name": res_ins[0], "sex": res_con[4], "ins_id": res_con[5],
            "pic": res_ins[1], "hCon_id": res_con[6], "hCon_name": res_hCon[0] + " " + res_hCon[1]}
    return token, info


def select_con_dashboard(conn, cursor, order_data, info):
    query = 'SELECT ins_id FROM ERNew.dbo.con WHERE user_id = ?'
...
def select_con_student_pf(conn, cursor, order_data, info):
    if order_data["kind"] == 0:
        query = 'SELECT user_id, first_name, last_name, gl_access, fr_access, ag_access, finalized, con_finalized, field, glf_access FROM ERNew.dbo.stu WHERE con_id = ? order by created_time desc'
    elif order_data["kind"] == 1:
        query = 'SELECT user_id, first_name, last_name, gl_access, fr_access, ag_access, finalized, con_finalized, field, glf_access FROM ERNew.dbo.stu WHERE con_id = ? and gl_access = 1 order by created_time desc'
    elif order_data["kind"] == 2:
        query = 'SELECT user_id, first_name, last_name, gl_access, fr_access, ag_access, finalized, con_finalized, field, glf_access FROM ERNew.dbo.stu WHERE con_id = ? and fr_access = 1 order by created_time desc'
    elif order_data["kind"] == 3:
        query = 'SELECT user_id, first_name, last_name, gl_access, fr_access, ag_access, finalized, con_finalized, field, glf_access FROM ERNew.dbo.stu WHERE con_id = ? and glf_access = 1 order by created_time desc'
    else:
        query = 'SELECT user_id, first_name, last_name, gl_access, fr_access, ag_access, finalized, con_finalized, field, glf_access FROM ERNew.dbo.stu WHERE con_id = ? and ag_access = 1 order by created_time desc'
    res_stu = db_helper.search_allin_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    stu_data = []
    if len(res_stu) == 0:
        token = str(uuid.uuid4())
    ...
    for stu in res_stu:
        s = {"name": stu[1] + " " + stu[2], "user_id": stu[0], "GL": stu[3], "FR": stu[4], "AG": stu[5],
             "finalized": stu[6], "con_finalized": stu[7], "field": stu[8], "GLF": stu[9]}
        stu_data.append(s)
    token = str(uuid.uuid4())
    return token, stu_data
...
def select_con_report_pf(conn, cursor, order_data, info):
    query = 'SELECT user_id, phone, first_name, last_name, field FROM ERNew.dbo.stu WHERE con_id = ? and ag_access = 1 order by created_time desc'
    res_stu = db_helper.search_allin_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    stu_data = []
    if len(res_stu) == 0:
        token = str(uuid.uuid4())
        return token, stu_data
    for stu in res_stu:
        s = {"name": stu[2] + " " + stu[3], "user_id": stu[0], "field": stu[4], "phone": stu[1]}
        stu_data.append(s)
    token = str(uuid.uuid4())
    return token, stu_data
...
def update_con_user_profile(conn, cursor, order_data, info):
    ...
    if row_count > 0:
        token = str(uuid.uuid4())
        return token, "اطلاعات با موفقیت تغییر یافت.", order_data["first_name"], order_data["last_name"]
    else:
        return None, "اطلاعات کاربر تغییر نیافت.", order_data["first_name"], order_data["last_name"]


def insert_con_add_stu(conn, cursor, order_data, info):
    ...
    query = 'SELECT ins_id, hCon_id FROM ERNew.dbo.con WHERE user_id = ?'
    res_con = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    if res_con is None:
    ...
    values = (
        ...
        int(order_data["quota"]), res_con[0], res_con[1], info["user_id"], password,
...
def update_con_stu(conn, cursor, order_data, info):
    query = 'SELECT con_id, finalized FROM ERNew.dbo.stu WHERE user_id = ?'
...
def update_con_stu_pf_finalize(conn, cursor, order_data, info):
    query = 'SELECT con_id, con_finalized FROM ERNew.dbo.stu WHERE user_id = ?'
    res_stu = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=order_data["stu_id"])

...
import json
import math
from Helper.func_helper import password_format_check, check_security_code, random_with_n_digits, update_step_hoshmand
from Users.FieldPick.field_pick import get_majors, get_universities, get_cities, search_fields, get_provinces, \
    get_exam_types, get_majors_fr, get_provinces_fr, get_universities_fr, \
    search_fields_fr, update_spfr, get_spfr, update_trfr, get_trfr, update_spfrb, get_spfrb, update_trfrb, get_trfrb, \
    get_majors_frb, get_provinces_frb, get_universities_frb, search_fields_frb
from Users.Quiz.quiz import submit_quiz_answer, select_stu_quiz_table_info, select_stu_quiz_info
from Users.Student.student import select_student_info, update_stu_password, update_stu_user_profile, insert_student, \
    select_student_login, update_stu_info
...
def check_token(conn, cursor, data):
    query = "SELECT token FROM ERS.dbo.tokens WHERE user_id = ?"
    res = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=data[0])
    if res is None:
        token = str(uuid.uuid4())
        field = '([token], [user_id], [phone])'
        values = (token, data[0], data[1],)
        db_helper.insert_value(conn=conn, cursor=cursor, table_name="tokens", fields=field,
                               values=values)
        return token
    else:
        return res[0]
...
def signin(conn, cursor, order_data):
    ...
    query = 'SELECT * FROM ERS.dbo.users WHERE phone = ?'
    ...
    if res is None:
        ...
        return {"status": 200, "tracking_code": None, "method_type": method_type,
                "error": " کاربری با این شماره تلفن موجود نمی‌باشد."}
    else:
        db_password = res[2]
        if db_password == password:
            token_user = check_token(conn, cursor, res)
        else:
            cursor.close()
            conn.close()
            return {"status": 200, "tracking_code": None, "method_type": method_type,
                    "error": "رمز عبور شما درست نمی‌باشد."}
        if res[3] == "stu":
            query = 'SELECT verify FROM ERS.dbo.stu WHERE phone = ?'
            res_verify = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=phone)
            if res_verify[0] == 0:
                cursor.close()
                conn.close()
                return {"status": 200, "tracking_code": None, "method_type": method_type,
                        "error": "شما هنوز احراز هویت انجام نداده‌اید."}
            token, stu_id, first_name, last_name, sex, city, birth_date, logo, sch_name, lock = select_student_login(
                conn, cursor, res[0])
            response_info = {"phone": phone, "user_id": res[0], "id": stu_id, "first_name": first_name,
                             "last_name": last_name, "sex": sex, "city": city, "birth_date": birth_date,
                             "role": res[3], "pic": logo, "sch_name": sch_name, "lock": lock}
            cursor.close()
            conn.close()
            return {"status": 200, "tracking_code": token_user, "method_type": method_type,
...
def update_user(conn, cursor, order_data, info):
    method_type = "UPDATE"
    token, data = update_stu_user_profile(conn, cursor, order_data, info)
    cursor.close()
    conn.close()
    return {"status": 200, "tracking_code": token, "method_type": method_type,
            "response": {"data": data,
...
def get_hoshmand_questions(conn, cursor, data, info):
    ...
    query = '''
        SELECT 
            ...
            obligation,
            method
        FROM ERS.dbo.hoshmand_questions 
        WHERE user_id = ?
        '''
    hoshmand_data = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    if not hoshmand_data:
...
def get_hoshmand_examtype(conn, cursor, data, info):
    ...
    query = '''
            SELECT 
                data,
                examtypes
            FROM ERS.dbo.hoshmand_examtype 
            WHERE user_id = ?
        '''
    hoshmand_data = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    query_question = 'select obligation, method from ERS.dbo.hoshmand_questions where user_id = ?'
    question = db_helper.search_table(conn=conn, cursor=cursor, query=query_question, field=info["user_id"])
    query_student = 'select sex, field, city, rank_zaban, rank_honar from ERS.dbo.stu where user_id = ?'
...
def get_hoshmand_province(conn, cursor, data, info):
    ...
    query = '''
            SELECT 
                data
            FROM ERS.dbo.hoshmand_province 
            WHERE user_id = ?
            '''
    hoshmand_data = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    query_student = 'select sex, field, city, rank_zaban, rank_honar from ERS.dbo.stu where user_id = ?'
    ...
    majors = []
    if not hoshmand_data:
        query_majors = 'select majors from ERS.dbo.hoshmand_major where user_id = ?'
        majors_res = db_helper.search_table(conn=conn, cursor=cursor, query=query_majors, field=info["user_id"])
        query_examtype = 'select examtypes from ERS.dbo.hoshmand_examtype where user_id = ?'
...
def get_hoshmand_tables(conn, cursor, data, info):
    ...
    if not hoshmand_data:
        ...
        query_majors = 'select major1, major2, major3, major4, majors from ERS.dbo.hoshmand_major where user_id = ?'
        ...
        query_province = 'select province1, province2, province3, province4, province5 from ERS.dbo.hoshmand_province where user_id = ?'
        ...
        uni_start = time.time()
        query_uni = 'select id from ERS.dbo.hoshmand_universities where user_id = ?'
...
def get_hoshmand_chains(conn, cursor, order_data, info):
    ...
    query = '''
           SELECT 
               chains,
               deleted_chains
           FROM ERS.dbo.hoshmand_chains 
           WHERE user_id = ?
           '''
    hoshmand_data = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    if not hoshmand_data:
        start_select = time.time()
        query_student = 'select sex, field, city, rank_zaban, rank_honar, quota from ERS.dbo.stu where user_id = ?'
        student = db_helper.search_table(conn=conn, cursor=cursor, query=query_student, field=info["user_id"])
        query_question = 'select obligation, method, univercity, major from ERS.dbo.hoshmand_questions where user_id = ?'
        question = db_helper.search_table(conn=conn, cursor=cursor, query=query_question, field=info["user_id"])
        field = str(student[1])
        if student[3] != 0 and student[3]:
            field += "," + str(4)
        ...
        values = (
            ...
            student[2].split(",")[0], str(question[0]), str(question[1]), 1404, sorting_major_uni, "ERS"
        )
        recs = []
...
def prepare_chain_data(conn, cursor, info, order_data):
    query_majors = 'SELECT major1, major2, major3, major4 FROM ERS.dbo.hoshmand_major WHERE user_id = ?'
    majors_result = db_helper.search_table(conn=conn, cursor=cursor, query=query_majors, field=info["user_id"])

    query_universities = 'SELECT uni1, uni2, uni3, uni4, uni5, uni6, uni7, uni8, uni9, uni10, uni11 FROM ERS.dbo.hoshmand_universities WHERE user_id = ?'
...
def get_hoshmand_chain_code(conn, cursor, order_data, info):
    method_type = "SELECT"
    query_student = 'select sex, field, city, rank, quota, rank_zaban, rank_honar from ERS.dbo.stu where user_id = ?'
    student = db_helper.search_table(conn=conn, cursor=cursor, query=query_student, field=info["user_id"])
    fields = []
    field = str(student[1])
...
def get_hoshmand_fields(conn, cursor, order_data, info):
    ...
    if not hoshmand_data:
        ...
        query_question = 'select obligation, method, univercity, major from ERS.dbo.hoshmand_questions where user_id = ?'
        question = db_helper.search_table(conn=conn, cursor=cursor, query=query_question, field=info["user_id"])
        query = 'SELECT suggested, other FROM ERS.dbo.hedayat_fields WHERE user_id = ?'
        ...
        values = (
            info["user_id"], field, student[4], student[3], 1404, suggested_fields, other_fields, "ERS"
...
def get_hoshmand_list(conn, cursor, order_data, info):
    ...
    '''
    hoshmand_data = db_helper.search_table(conn=conn, cursor=cursor, query=query, field=info["user_id"])
    query_student = 'select field, rank, quota, rank_zaban, rank_honar from ERS.dbo.stu where user_id = ?'
    student = db_helper.search_table(conn=conn, cursor=cursor, query=query_student, field=info["user_id"])
